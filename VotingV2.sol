...
contract Voting {
    ...
    //function to get wining candidate's Name
    function getWiningCandidate() public view returns (string memory candidateName_){

      //get wining candidate's index from getWiningCandidateIndex function
      uint candidateIndex = getWiningCandidateIndex();

      //check if wining index is valid and return candidate name
      if(candidateIndex < candidates.length){
        candidateName_ = candidates[candidateIndex].name;
      } else {
        candidateName_ = 'No wining Candidate';
      }
